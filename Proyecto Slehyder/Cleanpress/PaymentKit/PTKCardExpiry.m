...
{
    if (_month.length == 2 ) {
        return [NSString stringWithFormat:@"%@", [self formattedString1]];
    } else {
        return [self formattedString1];
    }
    
}

- (NSString *)formattedStringWithTral
{
...
- (NSUInteger)year
{
    ...
    if (yearStr.length == 2) {
        ...
        prefix = [prefix substringWithRange:NSMakeRange(0, 4)];
